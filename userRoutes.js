...
const ArrivedSafeNotification = require('./public/scripts/user/Functions')
...
// update appUsers when a user accepts a meeting
    ...
    .then(result => {
      if (ArrivedSafeNotification.groupLogic.groupExistsInCreatedGroup(result.recordset, groupName_)) {
        db.pools
          .then((pool) => {
            return pool.request()
              .query(`SELECT m.userName_ID, m.groupName_ID, m.nameOfPersonRequesting, m.TypeOfMeeting, m.date, m.time, m.meetingStatus, appUsers.email FROM meetingrequests AS m JOIN appUsers ON m.userName_ID = appUsers.userName WHERE groupName_ID = '${groupName_}'`)
          })
        // send back the result
          .then(_result => {
            const meetings = []
