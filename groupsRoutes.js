...
router.post('/api/joinByInvite', redirectLogIn, function (req, res) {
  ...
  db.pools
    .then((pool) => {
      return pool.request()
        .query(`SELECT existingGroup.userName_ID, appUsers.email FROM existingGroup INNER JOIN appUsers ON existingGroup.userName_ID = appUsers.userName WHERE groupName_ID = '${sessions.getActiveGroup()}'`)
    })
    .then(result => {
