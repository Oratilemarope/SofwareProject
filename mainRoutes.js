  ...
  db.pools
    ...
    .then((pool) => {
      ...
      if (Gcode === req.body.gcode) {
        return dbrequest
          .query('UPDATE appUsers SET password = @userp WHERE userName = @nam')
      } else {
  ...
  // Run query
    .then((pool) => {
      return pool.request()
        ...
        .query(`INSERT INTO appUsers(userName, email, firstName, lastName, studentNumber, CovidFlag, Adress, City, PostalCode, password) VALUES ('${studentObject.userName}', '${studentObject.emailAddress}', '${studentObject.firstName}', '${studentObject.lastName}', '${studentObject.studNumber}', NULL, NULL, NULL, NULL, '${studentObject.Password}')`)
    })
    // send back the result
    .then(_result => {
      res.redirect(req.baseUrl + '/user/homepage')
